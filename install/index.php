...
$user_to_files_sql ="CREATE TABLE IF NOT EXISTS `user_to_file` (
  ...
  PRIMARY KEY (`id`)
	...
	$stmt = $mysqli->prepare($documents_sql);
	if($stmt->execute())
	{
		echo "<p> documents table created.....</p>";
	}
	else
	{
		echo "<p>Error constructing documents table.</p>";
		$db_issue = true;
	}

$stmt = $mysqli->prepare($user_to_files_sql);
	if($stmt->execute())
	{
		echo "<p> user_to_files table created.....</p>";
	}
	else
	{
		echo "<p>Error constructing user_to_files table.</p>";
		$db_issue = true;
	}
